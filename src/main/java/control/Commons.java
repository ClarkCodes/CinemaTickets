...
{   // Commonly Used Objects for common tasks to centralize its declaration, declare them once, use them anywhere.
     ...
     */
    public static Commons getAdm () // M�todo est�tico y p�blico de esta clase que devuelve la instancia �nica de esta clase, si reci�n se abre la aplicaci�n y reci�n se van a agregar datos 
    {                                    //se crea la instancia de esta clase, pero si ya fu� creada se devuelve la que ya est�, manteniendo una �nica instancia siempre
        if ( adm == null )
            adm = new Commons();
        return adm;
    }
    ...
    /** Gets a formatted version of a given {@code String} of a one digit number
     * with the two digit format, this is with a 0 before that number.
     * 
     * @param oneDigit A {@code String} that should contain a one digit number
     * 
     * @return A formatted {@code String} with its contained number with the 2 
     * digit format, but exactly the same input if the given {@code String} has
     * its lenght greater than 1
     * @since 1.7
     */
    public static String doubleOneDigitStrFormatter( String oneDigit )
    {
        return oneDigit.length() == 1 ? "0" + oneDigit : oneDigit;
    }
